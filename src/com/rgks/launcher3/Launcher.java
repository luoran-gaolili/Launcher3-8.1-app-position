...
public class Launcher extends BaseActivity
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (isActionMain) {
            ...
            if (topOpenView instanceof PopupContainerWithArrow) {
                ...
                ued.logActionCommand(LauncherLogProto.Action.Command.HOME_INTENT,
                        ((Folder) topOpenView).getFolderIcon(), LauncherLogProto.ContainerType.FOLDER);
        ...
        if (topView != null) {
            if (topView.getActiveTextView() != null) {
                ...
                if (topView instanceof PopupContainerWithArrow) {
                    ued.logActionCommand(LauncherLogProto.Action.Command.BACK,
                            topView.getExtendedTouchView(), LauncherLogProto.ContainerType.DEEPSHORTCUTS);
                } else if (topView instanceof Folder) {
                    ued.logActionCommand(LauncherLogProto.Action.Command.BACK,
                            ((Folder) topView).getFolderIcon(), LauncherLogProto.ContainerType.FOLDER);
                }
                topView.close(true);
            }
        } else if (isAppsViewVisible()) {
            ued.logActionCommand(LauncherLogProto.Action.Command.BACK, LauncherLogProto.ContainerType.ALLAPPS);
            showWorkspace(true);
        } else if (isWidgetsViewVisible()) {
            ued.logActionCommand(LauncherLogProto.Action.Command.BACK, LauncherLogProto.ContainerType.WIDGETS);
            showOverviewMode(true);
        } else if (mWorkspace.isInOverviewMode()) {
            ued.logActionCommand(LauncherLogProto.Action.Command.BACK, LauncherLogProto.ContainerType.OVERVIEW);
            showWorkspace(true);
        } else {
            // TODO: Log this case.
            mWorkspace.exitWidgetResizeMode();
    ...
    protected void onClickAllAppsButton(View v) {
        ...
        if (!isAppsViewVisible()) {
            getUserEventDispatcher().logActionOnControl(LauncherLogProto.Action.Touch.TAP,
        ...
        if (v instanceof Workspace) {
            if (!mWorkspace.isInOverviewMode()) {
                if (!mWorkspace.isTouchActive() && !ignoreLongPressToOverview) {
                    getUserEventDispatcher().logActionOnContainer(LauncherLogProto.Action.Touch.LONGPRESS,
        ...
        if (!mDragController.isDragging()) {
            if (itemUnderLongClick == null) {
                ...
                if (mWorkspace.isInOverviewMode()) {
                    ...
                    getUserEventDispatcher().logActionOnContainer(LauncherLogProto.Action.Touch.LONGPRESS,
                            LauncherLogProto.Action.Direction.NONE, LauncherLogProto.ContainerType.OVERVIEW);
                } else {
