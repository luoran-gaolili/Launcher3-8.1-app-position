    ...
    protected CacheEntry cacheLocked(
        ...
        if (entry == null || (entry.isLowResIcon && !useLowResIcon)) {
            ...
            if (!getEntryFromDB(cacheKey, entry, useLowResIcon) || DEBUG_IGNORE_CACHE) {
                ...
                if (info != null) {
                    if(info.getApplicationInfo().packageName.equals("com.android.settings")){


                        //    LauncherAppState.getInstance(mContext).getLauncher().getHotseat().setBackground(info.getApplicationInfo().loadIcon(mPackageManager));

                    }

                    entry.icon = LauncherIcons.createIconBitmap(
                            getFullResIcon(info), mContext,1.0f
                          );
